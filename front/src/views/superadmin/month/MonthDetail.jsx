...
import { CheckersList } from "../../../components/checkersList/CheckersList";
...
export const MonthDetail = () => {
        ...
        try {
            ...
            const detailedRejas = await Promise.all(
                basicRejas.map(async (reja) => {
                    if (Array.isArray(reja.checkers)) {
    ...
    const getSortedCheckers = (reja) => {
        if (!reja.checkers?.length || !reja.checkerIds?.length) {
            return reja.checkers || [];
        }

        // Сортируем согласно порядку в checkerIds
        const checkerMap = new Map(reja.checkers.map(c => [String(c.id), c]));
        const sorted = reja.checkerIds
                                ...
                                rejas.map((reja, idx) => (
                                    <tr key={reja.id} className="transition hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-600">
                                            <CheckersList checkers={getSortedCheckers(reja)} />
                                        </td>
                                        <td className="whitespace-nowrap px-4 py-3 text-sm">
                                            {reja.example?.id ? (
                                                <a
