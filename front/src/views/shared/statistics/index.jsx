...
// RejaChecking status: 0=yuklandi/tekshiruvda (в процессе), 1=tasdiqlandi (одобрена), 2=rad etildi (отклонена)
// RejaHistory status: 0=pending (ожидает решения), 1=accepted (принято), 2=rejected (отклонено)
// Note: Holati shows THIS RejaHistory's status (checker's decision), not overall RejaChecking status
...
const getCourseValue = (item) => {
    const levelName =
        item.rejaChecking?.student?.level?.name ||
        item.rejaChecking?.student?.levelName ||
...
const RejaTekshirish = () => {
    ...
    const loadChecker = async () => {
        try {
            ...
            // Get current user ID and roles via decode
            ...
            const activeRole = decodeRes.data.activeRole;

            setCheckerId(userId);
            setUserRoles(activeRole ? [activeRole] : []);

            // Check if user is admin (ROLE_REKTOR or ROLE_SUPERADMIN)
            const isAdminUser = activeRole && (
                activeRole.authority === "ROLE_REKTOR" ||
                activeRole.authority === "ROLE_SUPERADMIN"
            );
            setIsAdmin(isAdminUser);
            console.log("✓ User role check:", { activeRole, isAdminUser });

            // Load histories based on role
            await loadHistories(userId, isAdminUser);
        } catch (err) {
    ...
    const loadHistories = async (userId, isAdminUser = false) => {
        try {
            ...
            if (isAdminUser) {
                // Load ALL RejaHistory for admin view
                res = await ApiCall(`/api/v1/reja-history/admin/all`, "GET");
            } else {
                // Get RejaHistory for this specific checker
                res = await ApiCall(`/api/v1/reja-history/by-checker/${userId}`, "GET");
            }
            ...
            if (!res.error && res.data) {
                ...
                const refreshedList = list.map(item => ({
                    ...item,
                    rejaChecking: item.rejaChecking ? { ...item.rejaChecking } : null,
                ...
                setHistories(sorted);
                console.log("✓ Histories loaded, total count:", sorted.length, "isAdmin:", isAdminUser);
            } else {
                setHistories([]);
            }
        } catch (err) {
    ...
    const handleSubmitAction = async () => {
        ...
        try {
            ...
            if (!res.error) {
                // Close modal first
                closeModal();
                // Reload histories to get updated RejaChecking status and next checker
                if (checkerId) {
                    console.log("✓ Action successful, reloading histories...");
                    await loadHistories(checkerId, isAdmin);
                ...
                <div className="mb-8 overflow-hidden rounded-2xl bg-white shadow-lg">
                    <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-6 sm:px-10">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-white">Reja tekshirish</h1>
                                <p className="mt-1 text-blue-200">
                                    {isAdmin
                                        ? "Barcha rejalarning tekshirish tarixini ko'rib chiqing"
                                        : "Sizga tayinlangan rejalarni ko'rib chiqing va tasdiqlang yoki rad eting"}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    {filteredHistories.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <h3 className="mt-4 text-lg font-semibold text-gray-700">
                                {filter !== "all" ? "Bu filtr bo'yicha rejalar topilmadi" : isAdmin ? "Rejalar topilmadi" : "Sizga tayinlangan rejalar yo'q"}
                            </h3>
                            <p className="mt-2 text-gray-500">
                                {isAdmin ? "Hozir tekshiriladigan rejalar yo'q" : "Yangi rejalar tayinlanganda ular bu yerda ko'rinadi."}
                            </p>
                        </div>
                    ) : (
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Doktorant</th>
                                        {isAdmin && (
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Tekshirilgan sana</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Amallar</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {paginatedHistories.map((item, idx) => {
                                        // Support both old system (RejaChecking) and new system (StudentRejaChecking)
                                        const isNewSystem = item.studentRejaChecking != null;

                                        const rejaName = isNewSystem
                                            ? (item.studentRejaChecking?.studentReja?.name || "—")
                                            : (item.rejaChecking?.reja?.name || item.rejaName || item.reja?.name || "—");

                                        const studentName = isNewSystem
                                            ? getPersonName(item.studentRejaChecking?.studentReja?.student)
                                            : (getPersonName(item.rejaChecking?.student) || item.studentName || getPersonName(item.student) || "—");

                                        const fileId = item.file?.id ||
                                            (isNewSystem ? item.studentRejaChecking?.file?.id : item.rejaChecking?.file?.id) ||
                                            null;

                                        const monthName = isNewSystem
                                            ? (item.studentRejaChecking?.studentReja?.month?.name || "—")
                                            : (item.rejaChecking?.reja?.month?.name || item.monthName || "—");
                                        ...
                                        return (
                                            <tr key={item.id} className="transition hover:bg-gray-50">
                                                ...
                                                {isAdmin && (
                                                    <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-600">
                                                        <div className="flex items-center gap-2">
                                                            <span className="inline-flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-purple-100 text-xs font-semibold text-purple-700">
                                                                <MdPerson className="h-4 w-4" />
                                                            </span>
                                                            <span>{getPersonName(item.checker)}</span>
                                                        </div>
                                                    </td>
                                                )}
                        ...
                        <div className={`flex items-center justify-between border-b px-6 py-4 ${modalAction === 2 ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"
                            ...
                            <div>
                                ...
                                <p className="mt-0.5 text-sm text-gray-600">
                                    {modalItem.rejaChecking?.reja?.name ||
                                     modalItem.studentRejaChecking?.studentReja?.name ||
                                     modalItem.rejaName ||
                                     modalItem.reja?.name || "Reja"}
                                </p>
                            </div>
                            <button
                                onClick={closeModal}
                        ...
                        <div className="px-6 py-5 space-y-4">
                            ...
                            <div className="rounded-xl bg-gray-50 p-4 space-y-2">
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-gray-500">Doktorant:</span>
                                    <span className="font-medium text-gray-800">
                                        {getPersonName(modalItem.rejaChecking?.student) ||
                                         getPersonName(modalItem.studentRejaChecking?.studentReja?.student) ||
                                         modalItem.studentName ||
                                         getPersonName(modalItem.student) || "—"}
                                    </span>
                                </div>
                                {(modalItem.rejaChecking?.file?.id || modalItem.studentRejaChecking?.file?.id || modalItem.file?.id) && (
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-gray-500">Fayl:</span>
                                        <button
                                            onClick={() => handleDownload(
                                                modalItem.rejaChecking?.file?.id ||
                                                modalItem.studentRejaChecking?.file?.id ||
                                                modalItem.file?.id,
                                                modalItem.rejaChecking?.reja?.name ||
                                                modalItem.studentRejaChecking?.studentReja?.name ||
                                                "fayl"
