  ...
  const loadHistories = async (userId) => {
    try {
      ...
      if (!res.error && res.data) {
        ...
        setHistories(sorted);
        console.log("✓ Histories loaded, total count:", sorted.length);
      } else {
        setHistories([]);
      }
    } catch (err) {
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {paginatedHistories.map((item, idx) => {
                    const rejaName =
                      item.rejaChecking?.reja?.name ||
                      item.rejaName ||
                      item.reja?.name ||
                      "—";
                    const studentName =
                      getPersonName(item.rejaChecking?.student) ||
                      item.studentName ||
                      getPersonName(item.student) ||
                      "—";
                    // Use file snapshot stored on this specific RejaHistory record.
                    // Falls back to rejaChecking.file only for old records that predate this field.
                    const fileId =
                      item.file?.id ||
                      item.rejaChecking?.file?.id ||
                      null;
            ...
            <div className={`flex items-center justify-between border-b px-6 py-4 ${modalAction === 2 ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-gray-600">
                  {modalItem.rejaChecking?.reja?.name || modalItem.rejaName || modalItem.reja?.name || "Reja"}
                </p>
              </div>
              <button
                onClick={closeModal}
              ...
              <div className="rounded-xl bg-gray-50 p-4 space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-800">
                    {getPersonName(modalItem.rejaChecking?.student) || modalItem.studentName || getPersonName(modalItem.student) || "—"}
                  </span>
                </div>
                {(modalItem.rejaChecking?.file?.id || modalItem.file?.id) && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500">Fayl:</span>
                    <button
                      onClick={() => handleDownload(
                        modalItem.rejaChecking?.file?.id || modalItem.file?.id,
                        modalItem.rejaChecking?.reja?.name || "fayl"
                      )}
