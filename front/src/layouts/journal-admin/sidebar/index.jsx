...
export default function JournalAdminSidebar({ open, onClose }) {
    ...
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-white shadow-lg transition-transform duration-300 dark:bg-gray-900 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
          ...
          <div className="mt-2 inline-flex items-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-semibold text-indigo-700">
            JURNAL ADMIN
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-6">
          <div className="space-y-1">
            {routes.filter((r) => r.layout === "/journal-admin" && !r.hidden).map((route, i) => {
              const fullPath = route.layout + "/" + route.path;
              return (
                <Link key={i} to={fullPath} onClick={handleClick}>
                  <div className={`flex cursor-pointer items-center rounded px-4 py-2 transition-colors ${isActive(fullPath) ? "bg-indigo-50 font-medium text-indigo-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"}`}>
